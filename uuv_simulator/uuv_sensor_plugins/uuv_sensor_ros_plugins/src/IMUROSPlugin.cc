...
void IMUROSPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  // Add the noise covariance to the Measurement message (used by the medusa stack)
  ...
  // Advertise an orientation topic to be used with the (medusa stack)
  std::string imuStackTopic = '/' + this->robotNamespace + "/measurement/orientation";
  this->medusaStackImuPub = this->rosNode->advertise<dsor_msgs::Measurement>(imuStackTopic, 1);
}
...
bool IMUROSPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  // Fill the ROS Measurement message (using NED standard) to be used by the Medusa stack
  ...
  this->medusaStackImuPub.publish(this->imuMeasurementMsg);
